--[[
    ...
    Quick start:
        local King = loadstring(game:HttpGet(".../KingAkbarUI.lua"))()
        local UI   = King({ Theme = "Royal" })
        local Win  = UI:Window({ Title = "King Panel" })
...
Theme.Palettes = {
    Royal = {  -- Signature theme (gold + deep navy)
        ...
        Accent         = Color3.fromRGB(220, 180, 90),   -- gold
        AccentHover    = Color3.fromRGB(240, 200, 110),
        AccentPress    = Color3.fromRGB(190, 150, 70),
        AccentAlt      = Color3.fromRGB(120, 90, 220),   -- royal purple
...
function Theme:Get(key)
    return self.Palette[key] or Color3.new(1, 1, 1)
end

--- Set warna custom (real-time propagation).
function Theme:Set(key, value)
    self.Palette[key] = value
    self:_emit()
end

--- Ganti mode tema ("Royal"|"Dark"|"Light"|"Midnight").
...
--- Toggle antar Dark & Light.
function Theme:ToggleMode()
    self:SetMode(self.Mode == "Dark" and "Light" or "Dark")
end
...
function Util.corner(parent, radius)
    return Util.create("UICorner", {
        CornerRadius = UDim.new(0, radius or 8), Parent = parent,
    })
end
...
--- Pulse glow pada elemen (untuk accent).
function Anims.pulse(frame, color)
    ...
    Util.corner(glow, 12)
    Util.tween(glow, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
...
function King:Window(opts)
    ...
    self.ThemeRef = opts.ThemeRef or Theme.new(opts.Theme or "Royal")
    self:_build(opts)
    self:_bindTheme()
    return self
end
...
function King:CreateWindow(opts)
    opts = opts or {}
    opts.ThemeRef = self.Theme
    local win = Window.new(self, opts)
    table.insert(self.Windows, win)
    return win
end

function Window.new(king, opts)
    local parent = opts.Parent
    if not parent then
        local ok, cg = pcall(function() return game:GetService("CoreGui") end)
        parent = ok and cg or LocalPlayer:WaitForChild("PlayerGui")
    ...
    self.King      = king
    self.UI        = king
    self.Aurora    = king -- compat
    self.Parent    = parent
    self.ThemeRef  = king.Theme
    self.Theme     = king.Theme
    self.Tabs      = {}
    self.Conn      = {}
    self.Minimized = false
    self:_build(opts)
...
function Window:_build(opts)
    ...
    -- Crown icon
    local crown = Util.create("TextLabel", {
        Name = "Crown",
    ...
    self.BtnTheme = makeCtrl("◐", nil, function() self.ThemeRef:ToggleMode() end)
    self.BtnMin   = makeCtrl("—", nil, function() self:ToggleMinimize() end)
    self.BtnClose = makeCtrl("✕", t:Get("Error"), function() self:Destroy() end)
...
function Window:Tab(opts)
    ...
    self2.Window    = self
    self2.Name      = opts.Name or "Tab"
    self2.Icon      = opts.Icon or ""
    self2.Items     = {}
    self2._opts     = opts
    ...
    if opts.Icon then
        ...
        self2.IconLabel = icon
    end

    local content = Util.create("Frame", {
        Name = "TabContent_" .. self2.Name,
    ...
    Util.list(content, Enum.FillDirection.Vertical, 8)
    self2.Container = content

    -- Hover & click
    btn.MouseEnter:Connect(function()
        if self.CurrentTab == self2 then return end
        Util.tween(btn, Config.Animation.Fast, {
            BackgroundTransparency = 0.6, BackgroundColor3 = t:Get("SurfaceHover"),
        })
        Util.tween(label, Config.Animation.Fast, { TextColor3 = t:Get("Text") })
        if self2.IconLabel then
            Util.tween(self2.IconLabel, Config.Animation.Fast, { TextColor3 = t:Get("Text") })
        end
    end)
    btn.MouseLeave:Connect(function()
        if self.CurrentTab == self2 then return end
        ...
        if self2.IconLabel then
            Util.tween(self2.IconLabel, Config.Animation.Fast, { TextColor3 = t:Get("TextMuted") })
        end
    end)
    ...
    if #self.Tabs == 1 then self:SelectTab(self2) end
    return self2
end
...
function Window:SelectTab(tab)
    ...
    Util.tween(tab.Button, Config.Animation.Fast, {
        BackgroundTransparency = 0, BackgroundColor3 = t:Get("SurfaceAlt"),
    })
...
function Tab:Button(opts)
    opts = opts or {}
    local t = self.Window.ThemeRef
    local style  = opts.Style or "Solid"     -- Solid | Outline | Gradient | Ghost
    ...
    local function applyHover(state)
        if style == "Solid" then
            Util.tween(btn, Config.Animation.Fast, {
                BackgroundColor3 = state and t:Get("AccentHover") or t:Get("Accent"),
            })
        elseif style == "Outline" then
            Util.tween(btn, Config.Animation.Fast, { BackgroundTransparency = state and 0.85 or 1 })
        elseif style == "Ghost" then
            Util.tween(btn, Config.Animation.Fast, { BackgroundTransparency = state and 0.85 or 1 })
        elseif style == "Gradient" then
            Util.tween(btn, Config.Animation.Fast, { BackgroundTransparency = state and 0 or 0.1 })
        end
    end

    btn.MouseEnter:Connect(function() applyHover(true) end)
    btn.MouseLeave:Connect(function() applyHover(false) end)
...
function Tab:Radio(opts)
    opts = opts or {}
    local t = self.Window.ThemeRef
    local groupName = opts.Group or Util.guid()
    ...
    local item = { row = row, circle = circle, dot = dot, value = opts.Value or opts.Name }
    ...
    item.deselect, item.select = deselect, select

    local function pick()
        if group.selected == item then return end
        if group.selected then group.selected.deselect() end
        group.selected = item
        select()
...
function Tab:Slider(opts)
    ...
    local isVertical = opts.Vertical == true

    local row = Util.create("Frame", {
        Size = UDim2.new(1, 0, 0, isVertical and 150 or 46),
        BackgroundTransparency = 1,
        Parent = self.Container,
    })
...
function Tab:Textbox(opts)
    ...
    local row = Util.create("Frame", {
        Size = UDim2.new(1, 0, 0, opts.Description and 48 or 40),
        BackgroundTransparency = 1,
        Parent = self.Container,
    })
    ...
    input.FocusLost:Connect(function(enterPressed)
        ...
        if opts.Callback then safeCall(opts.Callback, input.Text, enterPressed) end
    end)
...
-- @param opts { Title, Content, Duration, Type = "info"|"success"|"warning"|"error" }
function King:Notify(opts)
    ...
    local accents = {
        info = t:Get("Accent"), success = t:Get("Success"),
        warning = t:Get("Warning"), error = t:Get("Error"),
    }
